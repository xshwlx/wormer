...
(function ($) {
    ...
    $(function () {
        // code is copied form jQuery ($.browser is deprecated + there is a bug
        // in IE)
    ...
    $.jstree.plugin("core", {
        ...
        _fn: {
            ...
            prepare_move: function (o, r, pos, cb, is_cb) {
                ...
                p.p = (typeof p === "undefined") ? "last" : pos; // TODO:
                // move to a
                // setting
...
/*
 * jsTree ui plugin 1.0 This plugins handles
...
/*
 * jsTree CRRM plugin 1.0 Handles creating/renaming/removing/moving nodes by
 * user interaction.
 */
(function ($) {
    $.jstree.plugin("crrm", {
...
/*
 * jsTree themes plugin 1.0 Handles loading and setting themes, as well as
 * detecting path to themes, etc.
 */
(function ($) {
    var themes_loaded = [];
        ...
        if ($.jstree._themes === false) {
            $("script").each(function () {
                if (this.src.toString().match(/jquery\.jstree[^\/]*?\.js(\?.*)?$/)) {
                    $.jstree._themes = "css/themes/";// this.src.toString().replace(/jquery\.jstree[^\/]*?\.js(\?.*)?$/,
                    // "") + 'themes/';
                    return false;
                }
            });
...
/*
 * jsTree hotkeys plugin 1.0 Enables keyboard navigation for all tree instances
...
/*
 * jsTree JSON 1.0 The JSON data store. Datastores are build by overriding the
...
/*
 * jsTree languages plugin 1.0 Adds support for multiple language versions in
 * one tree This basically allows for many titles coexisting in one node, but
 * only one of them being visible at any given time This is useful for
 * maintaining the same structure in many languages (hence the name of the
 * plugin)
 */
(function ($) {
...
/*
 * jsTree cookies plugin 1.0 Stores the currently opened/selected nodes in a
 * cookie and then restores them Depends on the jquery.cookie plugin
 */
(function ($) {
...
/*
 * jsTree sort plugin 1.0 Sorts items alphabetically (or using any other
 * function)
 */
(function ($) {
    $.jstree.plugin("sort", {
...
/*
 * jsTree checkbox plugin 1.0 Inserts checkboxes in front of every node Depends
 * on the ui plugin DOES NOT WORK NICELY WITH MULTITREE DRAG'N'DROP
 */
(function ($) {
        ...
        _fn: {
            ...
            change_state: function (obj, state) {
                ...
                obj.parentsUntil(".jstree", "li").each(function () {
                    var $this = $(this);
                    if (state) {
                        if (obj.attr("mustSelected") == "true") {
                            var bro = obj.parent("ul").children(".jstree-checked, .jstree-undetermined");
                            if (bro.length > 0) {
                                $.each(bro, function (i, n) {
                                    $(n).find("li").andSelf().removeClass("jstree-checked jstree-undetermined").addClass("jstree-unchecked");
                                });
                            }
                        }
                        ...
                        var bro = $this.children("ul").children(".jstree-unchecked, .jstree-undetermined");
                        $.each(bro, function (i, n) {
                            if ($(n).attr("mustSelected") == "true") {
                                $(n).find("li").andSelf().removeClass("jstree-unchecked jstree-undetermined").addClass("jstree-checked");
                            }
                        });
                        var relates = obj.attr("relateModules");
                        if (typeof(relates) == "string") {
                            var codes = relates.split(",");
                            var len = codes.length;
                            for (var m = 0; m < len; m++) {
                                var id = "#" + codes[m];
                                if ($(id).hasClass("jstree-unchecked")) {
                                    _this.check_node($(id));
                                }
                            }
                        }
...
/*
 * jsTree XML 1.0 The XML data store. Datastores are build by overriding the
 * `load_node` and `_is_loaded` functions.
 */
(function ($) {
...
/*
 * jsTree search plugin 1.0 Enables both sync and async search on the tree DOES
 * NOT WORK WITH JSON PROGRESSIVE RENDER
 */
(function ($) {
...
/*
 * jsTree types plugin 1.0 Adds support types of nodes You can set an attribute
 * on each li node, that represents its type. According to the type setting the
...
/*
 * jsTree HTML data 1.0 The HTML data store. Datastores are build by replacing
...
/*
 * jsTree themeroller plugin 1.0 Adds support for jQuery UI themes. Include this
 * at the end of your plugins list, also make sure "themes" is not included.
 */
(function ($) {
...
/*
 * jsTree unique plugin 1.0 Forces different names amongst siblings (still a bit
